...
```
HEAD: 12d37ab25efa3f48906bca3773e333eecacbbf49
Branch: m16-t01-card-manifest
Git status: clean

Tag chain (oldest → newest):
  ...
  v3.5-m17-t01-hotfix2-system-menu-dock-button-layout  12d37ab  (hotfix 2) ← HEAD
```

---

## 2. Modified Files (planning freeze → HEAD)
...
### HEAD Re-run (commit 12d37ab)
```
M15-T01 caremodel: BASELINE_BIT_EQUIVALENCE=FAIL (expected — Rebaseline Protocol v2)
M15-T02 care_ui: PASS
M16-T01 manifest: PASS (26/0)
M16-T02 rescue_card: PASS (55P/16F — 16 expected: new species use placeholder source)
M16-T03 codex: PASS (35/0)
M14 rescue_core: FAIL (expected — species selection divergence)
M14-T02 rescue_ui: FAIL (expected — species selection divergence)
M14-T03 copy_ui: FAIL (expected — species selection divergence)
M13: Not re-run (economy tests, no code path to pool, 10min/test)
```

All HEAD failures are Rebaseline Protocol v2 expected divergences from pool 3→6.
...
- **New baseline**: HEAD (commit 12d37ab) → `data/species_rescue_pool.json` (6 entries)
